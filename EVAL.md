...
GLM-5.1 / GPT-5.5 / GPT-5.4 / DS-V4-Pro / MiMo 五家全部 100% recall + 100% precision。当前 40 条 golden 区分度不够。

**性价比排名**：GLM-5.1 开推理 (30k tokens, 4步) > MiMo (44k, 5步) > DS-V4-Pro (63k, 9步) > GPT-5.4 (145k, 9步) > GPT-5.5 (297k, 12步)
...
2. **Golden 上限**：当前 40 条区分度不够，5 家已达 100% 天花板
...
以下数据来自各 Agent 独立 eval，最后一次完整跑是 **2026-04-18**。
当时的 prompt 版本下 DesignDoc recall 仅 55.56%、TestGen v2 recall 仅 20%。
经过 prompt 强化后当前 recall 已达 100% / 80%（见上表）。

详细运行时数据见 [`evals/agent-eval-details.md`](evals/agent-eval-details.md)（`make eval` 自动生成）。
